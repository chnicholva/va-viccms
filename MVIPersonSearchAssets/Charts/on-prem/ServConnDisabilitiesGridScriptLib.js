...
function scdg_SettingsWebServiceURL_response(scdg_settingData, scdg_lastSkip, scdg_eesummaryURLbase_NA) {
    try {
        //scdg_lastSkip is the starting point in the result (use if more than 50 records) //Not used in this scenario
        var scdg_DacUrl = null;
        var scdg_EnrollmentEligibilitySummaryApiUrl = null;
        for (var i = 0; i <= scdg_settingData.d.results.length - 1; i++) {
            //Get info
            if (scdg_settingData.d.results[i].ftp_DACURL != null) { scdg_DacUrl = scdg_settingData.d.results[i].ftp_DACURL; }
            if (scdg_settingData.d.results[i].ftp_ESREnrollmentEligibilitySummaryAPIURL != null) { scdg_EnrollmentEligibilitySummaryApiUrl = scdg_settingData.d.results[i].ftp_ESREnrollmentEligibilitySummaryAPIURL; }
            break;
        }

        if (scdg_DacUrl != null && scdg_EnrollmentEligibilitySummaryApiUrl != null) {
            //Construct full web service URL
            scdg_EnrollmentEligibilitySummaryURLbase = scdg_DacUrl + scdg_EnrollmentEligibilitySummaryApiUrl;
            ...
            var scdg_idobject = {};
            scdg_idobject.NationalId = '000000' + scdg_xrmdata + '000000';
            var scdg_serviceParams = [{
                key: "identifier",
                type: "c:string",
                value: JSON.stringify(scdg_idobject)
            }];
            //Call the web service security function
            CrmSecurityTokenEncryption(scdg_EnrollmentEligibilitySummaryURLbase, scdg_serviceParams, scdg_serverUrl, scdg_getEsrEnrollmentJSON_response)
        }
        else {
...
function scdg_getEsrEnrollmentJSON_response(scdg_error, scdg_getesrresponse) {
    try {
        //NOTE:Expecting JSON result, if changed to XML modify code accordingly.

        //Set JSON Data to Null;
        scdg_EnrollmentEligibilitySummaryData = null;

        //Check for non ESR service error
        if (scdg_error != null) {
            alert("An ESR Secure Service error occured:\n " + scdg_error + " \nDebugInfo: " + scdg_getesrresponse);
        }
        else {
            //Test for ESR service error
            if (scdg_getesrresponse.ErrorOccurred == true) {
                alert("An ESR Secure Service error occurred:\n " + scdg_getesrresponse.ErrorMessage + " \nDebugInfo: " + scdg_getesrresponse.DebugInfo);
            }
            else {
                //Set Data Returned
                scdg_EnrollmentEligibilitySummaryData = scdg_getesrresponse;
            }
        }
        //Build the Grid
        scdg_BuildGrid();
