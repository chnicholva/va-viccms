...
XrmServiceToolkit.Rest = function () {
    ...
    var errorHandler = function (req) {
        ...
        //req.statusText + ": " +
        //JSON.parse(req.responseText).error.message.value;
        return JSON.parse(req.responseText).error.message.value;
    };
...
XrmServiceToolkit.Soap = function () {
                        ...
                        for (var k = 0, lenk = attr.childNodes.length; k < lenk; k++) {
                            ...
                            switch (sType) {
                                ...
                                case "a:EntityReference":
                                    ...
                                    if (getNodeText(oChildNodes[1]) != null && getNodeText(oChildNodes[1]) != undefined && getNodeText(oChildNodes[1]) != "") {
    ...
    var fetchNext = function (fetchXml, pageNumber, pageCount, pageCookie) {

        var fetchResults = [];

        //Build new query
        var moreFetchXml = addPaging(fetchXml, pageNumber, pageCount, pageCookie);

        //the XML encodes & as &amp; which is not understood by CRM so replace it with &
        moreFetchXml = moreFetchXml.replace(/&amp;/g, '&');
        ...
        return doRequest(msgBody, "RetrieveMultiple", false, function (resultXml) {
            var newFetchResult;
            if ($(resultXml).find("a\\:Entities").length != 0) {
                newFetchResult = $(resultXml).find("a\\:Entities").eq(0)[0];
            } else {
                newFetchResult = $(resultXml).find("Entities").eq(0)[0]; //chrome
            }

            var newMoreRecords;
            if ($(resultXml).find("a\\:MoreRecords").length != 0) {
                if ($(resultXml).find("a\\:MoreRecords").eq(0)[0].firstChild.textContent != undefined) {
                    newMoreRecords = ($(resultXml).find("a\\:MoreRecords").eq(0)[0].firstChild.textContent === "true");
                }
                else {
                    newMoreRecords = ($(resultXml).find("a\\:MoreRecords").eq(0)[0].text === "true");
                }
            } else {
                // newMoreRecords = ($(resultXml).find("MoreRecords").eq(0)[0].firstChild.textContent === "true"); //chrome
                if ($(resultXml).find("MoreRecords").eq(0)[0].firstChild.textContent != undefined) {
                    newMoreRecords = ($(resultXml).find("MoreRecords").eq(0)[0].firstChild.textContent === "true");
                }
                else {
                    newMoreRecords = ($(resultXml).find("MoreRecords").eq(0)[0].text === "true");
                }
            }

            //var pageCookie = selectSingleNodeText(resultXml, "//a:PagingCookie").replace(/\"/g, '\'').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\'/g, '&quot;').replace(/\*/g, '&#42;');//.replace(/@/g, '&#64;').replace(/#/g, '&#35;').replace(/!/g, '&#33;').replace(/$/g, '&#36;').replace(/=/g, '&#61;').replace(/^/g, '&#94;'); //.replace(/:/g, '&#58;')

            var pageCookie = xmlEncode(selectSingleNodeText(resultXml, "//a:PagingCookie"));
            ...
            fetchResultObj = { fetchResult: fetchResults, fetchMoreRecords: newMoreRecords, fetchPageCookie: pageCookie };

            return fetchResultObj;

            //if (!async)
            //    return fetchResultObj;
            //else
            //    callback(fetchResultObj);

            //if (newMoreRecords == "true") {
            //    pageNumber += 1;
            //    var newPageCookie;
            //    if ($(resultXml).find("a\\:PagingCookie").length != 0) {
            //        newPageCookie = $(resultXml).find("a\\:PagingCookie").eq(0)[0].firstChild.textContent.replace(/\"/g, '\'').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/'/g, '&quot;');
            //    } else {
            //        newPageCookie = $(resultXml).find("PagingCookie").eq(0)[0].firstChild.textContent.replace(/\"/g, '\'').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/'/g, '&quot;');
            //    }

            //    fetchNext(fetchXml, pageNumber, newPageCookie, fetchResults, async, callback);
    ...
    function addPaging(fetchxml, page, pageCount, pagingCookie) {
        try {
            var parser = new DOMParser();
    ...
    var fetchPageWise = function (fetchCore, pageNumber, pageCount, pageCookie, callback) {
        //Build new query      
        var fetchXml = fetchCore;
        ...
        if (fetchCore.slice(0, 7) === "<entity") {
            fetchXml =
                 [
                     "<fetch mapping='logical' page='" + pageNumber + "' count='" + pageCount + "' paging-cookie='" + pageCookie + "'>",
                    // "<fetch mapping='logical' page='" + pageNumber + "' count='" + pageCount + "'>",
                     fetchCore.replace(/\"/g, "'"),
                     "</fetch>"
                 ].join("")
        } else {
            ...
            if (!isAggregate && !isLimitedReturn) {
                fetchXml =
                 [
                     "<fetch mapping='logical' page='" + pageNumber + "' count='" + pageCount + "' paging-cookie='" + pageCookie + "' distinct='" + (isDistinct ? distinctValue : "false") + "' >",
                     //"<fetch mapping='logical' page='" + pageNumber + "' count='" + pageCount + "' distinct='" + (isDistinct ? distinctValue : "false") + "' >",
                     fetchCore,
                     "</fetch>"
                 ].join("");
            }
        ...
        return doRequest(request, "Execute", async, function (resultXml) {
            var fetchResultObj;
            var fetchRes = selectSingleNode(resultXml, "//a:Entities");
            var moreRecords = (selectSingleNodeText(resultXml, "//a:MoreRecords") === "true");
            var pageCookie = selectSingleNodeText(resultXml, "//a:PagingCookie").replace(/\"/g, '\'').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/'/g, '&quot;');

            var fetchResults = [];
            if (fetchRes != null) {
                for (var ii = 0, olength = fetchRes.childNodes.length; ii < olength; ii++) {
                    var entity = new businessEntity();

                    entity.deserialize(fetchRes.childNodes[ii]);
                    fetchResults.push(entity);
                }

                fetchResultObj = { fetchResult: fetchResults, fetchMoreRecords: moreRecords, fetchPageCookie: pageCookie };
