...
XrmServiceToolkit.Soap = function () {
    ...
    var fetchAll = function (fetchXml, callback) {
        ///<summary>
        /// Sends synchronous/asynchronous request to do a fetch request.
        ///</summary>
        ///<param name="fetchXml" type="String">
        ...
        return doRequest(msgBody, "RetrieveMultiple", async, function (resultXml) {
            ...
            if ($(resultXml).find("a\\:MoreRecords").length != 0) {
                moreRecords = $(resultXml).find("a\\:MoreRecords").eq(0)[0].firstChild.textContent; //$(resultXml).find("a\\:MoreRecords").eq(0)[0].firstChild.text === "true";
            } else {
                moreRecords = $(resultXml).find("MoreRecords").eq(0)[0].firstChild.textContent; //$(resultXml).find("MoreRecords").eq(0)[0].firstChild.text === "true"; //chrome
            }
    ...
    var fetchNext = function (fetchXml, pageNumber, pageCookie, fetchResults, async, callback) {

        //Build new query
        var moreFetchXml = addPaging(fetchXml, pageNumber, pageCookie);
        ...
        return doRequest(msgBody, "RetrieveMultiple", false, function (resultXml) {
            ...
            var newMoreRecords;
            if ($(resultXml).find("a\\:MoreRecords").length != 0) {
                newMoreRecords = $(resultXml).find("a\\:MoreRecords").eq(0)[0].firstChild.textContent;
            } else {
                newMoreRecords = $(resultXml).find("MoreRecords").eq(0)[0].firstChild.textContent; //chrome
            }
    ...
    //function addPaging(fetchxml, page, pagingCookie) {

    //    // add the paging attributes
